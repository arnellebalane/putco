  ...
  postSelectedText() {
    ...
    if (selectedText) {
      var request = new Request('http://putco.de/_save', {
        ...
        body: `data=${encodeURIComponent(selectedText)}`
      });
      fetch(request)
        ...
        .then(function(response) {
          var extension = editor.getTitle().split('.').pop();
          var url = `http://putco.de/${response.name}.${extension}`;
          atom.clipboard.write(url);
          atom.notifications.addInfo(`Copied: <b>${url}</b>`);
        });
    }
  }
